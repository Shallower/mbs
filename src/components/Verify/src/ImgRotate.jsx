...
export default defineComponent({
    ...
    setup(props, { emit, attrs }) {
        ...
        return () => {
            ...
            const time = (endTime - startTime) / 1000;
            return (<div class="ir-dv">
          <div class={`ir-dv-img__wrap`} style={unref(getImgWrapStyleRef)}>
            <img src={src} onLoad={handleImgOnLoad} width={parseInt(props.width)} class={imgCls} style={state.imgStyle} onClick={() => {
                resume();
            }}/>
            ...
            {!state.showTip && !state.draged && (<span class={[`ir-dv-img__tip`, 'normal']}>t('redoTip')</span>)}
          </div>
          <BasicDragVerify class={`ir-dv-drag__bar`} onMove={handleDragBarMove} onEnd={handleDragEnd} onStart={handleStart} ref={basicRef} {...{ ...attrs, ...props }} value={isPassing} isSlot={true}/>
        </div>);
        };
    },
});
//# sourceMappingURL=ImgRotate.jsx.map