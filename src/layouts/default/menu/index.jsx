...
export default defineComponent({
    ...
    setup(props) {
        const go = useGo();
        const { getMenuMode, getMenuType, getMenuTheme, getCollapsed, getCollapsedShowTitle, getAccordion, getIsHorizontal, getIsSidebarType, } = useMenuSetting();
        ...
        function renderMenu() {
            ...
            if (!menus || !menus.length)
                return null;
            return !props.isHorizontal ? (<SimpleMenu beforeClickFn={beforeMenuClickFn} items={menus} theme={unref(getComputedMenuTheme)} accordion={unref(getAccordion)} collapse={unref(getCollapsed)} collapsedShowTitle={unref(getCollapsedShowTitle)} onMenuClick={handleMenuClick}/>) : (<BasicMenu beforeClickFn={beforeMenuClickFn} isHorizontal={props.isHorizontal} type={unref(getMenuType)} collapsedShowTitle={unref(getCollapsedShowTitle)} showLogo={unref(getIsShowLogo)} mode={unref(getComputedMenuMode)} theme={unref(getComputedMenuTheme)} items={menus} accordion={unref(getAccordion)} onMenuClick={handleMenuClick}/>);
        }
        return () => {
            return (<>
          {renderHeader()}
